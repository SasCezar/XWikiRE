...
options = SimpleNamespace(
    ...
    discardElements=[
        ...
        'img', 'imagemap', 'ref', 'references','source', 'small',
        'sub', 'sup', 'indicator'
    ],
...
class Extractor(object):
    ...
    def __init__(self, id, revid, title, lines, wikidata_id):
        """
        :param id: id of page.
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            if self.wikidata_id:
                json_data['wikidata_id'] = self.wikidata_id
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
                 process_count, wikidata_mapping=""):
    """
    ...
    for i in range(worker_count):
        extractor = Process(target=extract_process,
                            args=(options, i, jobs_queue, output_queue, wikidata_mapping_dict))
...
def extract_process(opts, i, jobs_queue, output_queue, wikidata_mapping=None):
    ...
    while True:
        ...
        if job:
            ...
            wikidata_id = None
            if wikidata_mapping:
                if id in wikidata_mapping:
                    wikidata_id = wikidata_mapping[id]
                else:
                    logging.info('Page with ID %s and title %s was not found in the wikidata mapping', id, title)
                    text = ''
                    output_queue.put((page_num, text))
                    out.truncate(0)
                    out.seek(0)
            try:
                e = Extractor(id, revid, title, page, wikidata_id)  # (id, revid, title, page)
                page = None  # free memory
                e.extract(out)
                text = out.getvalue()
            except:
...
def main():
    ...
    groupP.add_argument("--wikidata_mapping", default="", help="Maps wikipedia ids to wikidata ids. The file is a dictionary")
    ...
    options.wikidata_mapping = args.wikidata_mapping
    if args.html:
    ...
    process_dump(input_file, args.templates, output_path, file_size,
                 args.compress, args.processes, args.wikidata_mapping)
