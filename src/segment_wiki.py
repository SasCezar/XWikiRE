...
WIKIIMAGE_REGEX = re.compile(b"^\[{2}[^:\n<]+:[^\.\n<]+\.\w+[^\n<]+\]{2,}", re.MULTILINE)
WIKIIMAGES_REGEX = re.compile(b"\{\{\s*images(\s|\n)*(\|[^|}]+)*(?=\}{2})\}\}\n")
...
def segment(page_xml, include_interlinks=False, clean=True):
    ...
    wikipedia_id_path = "./{%(ns)s}id" % ns_mapping
    ...
    wiki_id = elem.find(wikipedia_id_path).text
    ...
    if include_interlinks:
        return wiki_id, title, sections, interlinks
    else:
        return wiki_id, title, sections
    ...
    def get_texts_with_sections(self):
        ...
        for group in utils.chunkize(page_xmls, chunksize=10 * self.processes, maxsize=1):
            for article in pool.imap(partial(segment, include_interlinks=self.include_interlinks),
                ...
                article_id, article_title, sections = article[0], article[1], article[2]
                ...
                if self.include_interlinks:
                    interlinks = article[3]
                    yield (article_id, article_title, sections, interlinks)
                else:
                    yield (article_id, article_title, sections)
